    ...
    $("#btn-normalisasi").on("click", function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                `;
                if (keys.length == 0) {
                    normalisasi += `<tr><td class="text-center" colspan="${2 + data.sum_kriteria}">Belum Ada Perhitungan</td></tr></tbody></table></div>`
                } else {
                    normalisasi += data.elements + '</tbody></table></div>'
                }
                ...
                ranking += `</tbody></table></div></div>`
                ranking += `<div class="card-footer">
                <a class="btn btn-primary" href="/cetak-laporan" target="_blank">Cetak Hasil</a>
                </div>`
                rankingElement.innerHTML = ranking
                $("#spinner").html("")

                console.log(response.pecah_hasil)
            }
        });
    })
